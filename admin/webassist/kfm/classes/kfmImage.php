...
class kfmImage extends kfmFile{
	...
	function getImageId(){
		...
		if(!$row){ # db record not found. create it
			# TODO: retrieve caption generation code from get_thumb.php
	...
	function setThumbnail($width=64,$height=64,$hue=0,$saturation=0,$lightness=0){
		...
		$this->thumb_url='get_thumb.php?type=thumb&id='.$id.GET_PARAMS;
		$this->thumb_id=$id;
	...
	function useImageMagick($from,$action,$to){
		$checkFileExists = @file_exists(IMAGEMAGICK_PATH);
