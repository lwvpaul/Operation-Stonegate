   ...
   */
   ...
   function getContent(){
	...
	function getUrl($x=0,$y=0){
       ...
       The url can be constructed since kfm_url should be given and get_thumb.php is in that root */
    global $kfm_userfiles_output;
		if(preg_replace('/.*(get\.php)$/','$1',$kfm_userfiles_output)=='get_thumb.php'){
			if($kfm_userfiles_output=='get_thumb.php')$url=preg_replace('/\/[^\/]*$/','/get_thumb.php?id='.$this->id.GET_PARAMS,$_SERVER['REQUEST_URI']);
    ...
    /* end deprication block */
		...
		if($kfm->setting('file_url')=='secure'){
			$url=$kfm->setting('kfm_url').'get_thumb.php?id='.$this->id.GET_PARAMS;
		}
		else{
