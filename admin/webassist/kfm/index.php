...
function rmkdirkfm($path, $mode = 0755) {
...
if(isset($_GET['startup_folder']) && strpos($_GET['startup_folder'],"/") === 0) {
		...
		$_SESSION['useOverrideRoot'] = (dirname(dirname(dirname(__FILE__)))) . $_GET['startup_folder'];
} else if (isset($_GET['theme']))  {
		unset($_SESSION['useOverrideRoot']);
}
...
<html>
    ...
    <link type="text/css" rel="stylesheet" href="http://ajax.googleapis.com/ajax/libs/jqueryui/1.7.2/themes/ui-lightness/jquery-ui.css" />
		<style type="text/css">@import "themes/<?php echo $kfm->setting('theme'); ?>/css.php?uicolor=<?php echo(isset($_GET['uicolor'])?str_replace("#","%23",$_GET['uicolor']):"%23eee"); ?>"; </style>
		<style type="text/css">@import "plugins/css.php"; </style>
		<title>File Manager - Kae's File Manager</title>
	</head>
	<body>
		<div id="removeme">
...
if (file_exists('themes/'.$kfm->setting('theme').'/template.html')) {
	...
	if (isset($kfm_hidden_sidebar) && $kfm_hidden_sidebar==true)  {
		$template = str_replace("[hideside]","display:none;",$template);
		$template = str_replace("[hidemargin]","margin-left: 0;",$template);
	} else {
		$template = str_replace("[hideside]","",$template);
		$template = str_replace("[hidemargin]","",$template);
