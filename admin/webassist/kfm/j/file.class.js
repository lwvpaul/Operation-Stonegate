...
File.prototype.setThumbnailBackground=function(el,reset){
	...
	el.style.background='url(get_thumb.php?id='+id+'&width=64&height=64'+kfm_vars.get_params + addRand + ') center top no-repeat transparent';
	
	if (el.childNodes && el.childNodes[0] && (selectedFiles.length>0)) {
		el.childNodes[0].style.background='url(get_thumb.php?id='+id+'&width=64&height=64'+kfm_vars.get_params + addRand + ') center top no-repeat transparent'
		el.style.background='';
...
File.prototype.iterateThumbnailQueue=function(){
	...
	if(el && el.parentNode && el.parentNode.id=='documents_body'){
		var url='get_thumb.php?id='+id+'&width=64&height=64'+kfm_vars.get_params;
