...
if ((isset($_POST["WADbSearch1"])) && ($_POST["WADbSearch1"] != "")) {
  ...
  if (1 == 1) {
    $_SESSION["WADbSearch1_MasterCQWDelete"]=$WADbSearch1->whereClause;
  }
}
else     {
  $WADbSearch1 = new FilterDef;
  ...
  if (1 == 1)     {
    if (isset($_SESSION["WADbSearch1_MasterCQWDelete"]) && $_SESSION["WADbSearch1_MasterCQWDelete"] != "")     {
      $WADbSearch1->whereClause = $_SESSION["WADbSearch1_MasterCQWDelete"];
    }
    else     {
...
$query_rs_CQWDelete = "SELECT * FROM admin_media_cqw";
setQueryBuilderSource($query_rs_CQWDelete,$WADbSearch1,false);
$rs_CQWDelete = mysql_query($query_rs_CQWDelete, $GoCreate) or die(mysql_error());
$row_rs_CQWDelete = mysql_fetch_assoc($rs_CQWDelete);
$totalRows_rs_CQWDelete = mysql_num_rows($rs_CQWDelete);?>
<?php
...
<!-- leftcolumn Content Ends Here -->
          ...
          <div class='cssLO GoCreateAdmin1_centercolumn_layout'>
            <div class='centercolumn cssLI GoCreateAdmin1_centercolumn_design'>
              ...
              <div id="HeaderTitle">Master C/Q/W Delete for <span class="HighLight"><?php echo $_GET['MT'];?></span> size <span class="HighLight" ><?php echo $_GET['MS'];?></span></div>
...
<?php
	...
	while($RepeatSelectionCounter_1_IterationsRemaining--){
		if($RepeatSelectionCounterBasedLooping_1 || $row_rs_CQWDelete){
?>
              <tr align="right" class="roweffect">
                    <td class="form_txt"><input type="hidden" name="WADA_RepeatID_ID_<?php echo $RepeatSelectionCounter_1; ?>" id="WADA_RepeatID_ID_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWDelete["ID"]; ?>" />
                      <input name="CQW_ID_<?php echo $RepeatSelectionCounter_1; ?>" type="hidden" id="CQW_ID_<?php echo $RepeatSelectionCounter_1; ?>" value='<?php echo $row_rs_CQWDelete["ID"]; ?>' />
<?php echo $RepeatSelectionCounter_1+1; ?></td>
                    <td class="form_txt">&pound;&nbsp;
                  <input name="Cost_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="Cost_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWDelete['COST']; ?>" size="12" /></td>
                    <td><input name="QTY_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="QTY_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWDelete['QTY']; ?>" size="12" /></td>
                    <td><select name="SetQTY_<?php echo $RepeatSelectionCounter_1; ?>" class="form" id="SetQTY__<?php echo $RepeatSelectionCounter_1; ?>">
                      <option selected="selected" value="" <?php if (!(strcmp("", $row_rs_CQWDelete['LIMIT']))) {echo "selected=\"selected\"";} ?>>Select..</option>
                      <option value="SetQTY" <?php if (!(strcmp("SetQTY", $row_rs_CQWDelete['LIMIT']))) {echo "selected=\"selected\"";} ?>>Set Quantity</option>
                      <option value="UpToInc" <?php if (!(strcmp("UpToInc", $row_rs_CQWDelete['LIMIT']))) {echo "selected=\"selected\"";} ?>>Up To &amp; Inc</option>
                      <option value="Single" <?php if (!(strcmp("Single", $row_rs_CQWDelete['LIMIT']))) {echo "selected=\"selected\"";} ?>>Single</option>
                    </select></td>
                    <td><input name="Weight_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="Weight_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWDelete['WEIGHT']; ?>" size="12" />
...
<?php } // RepeatSelectionCounter_1 End Alternate Content
		if(!$RepeatSelectionCounterBasedLooping_1 && $RepeatSelectionCounter_1_IterationsRemaining != 0){
			if(!$row_rs_CQWDelete && $RepeatSelectionCounter_1_Iterations == -1){$RepeatSelectionCounter_1_IterationsRemaining = 0;}
			$row_rs_CQWDelete = mysql_fetch_assoc($rs_CQWDelete);
		}
...
?>
<tr>
                    <td colspan="5" align="right" class="form_txt"><input name="ConfirmDeletion" type="submit" class="cust_button" id="ConfirmDeletion" value="ConfirmDeletion" />
                      &nbsp;
...
<?php
mysql_free_result($rs_CQWDelete);
?>
