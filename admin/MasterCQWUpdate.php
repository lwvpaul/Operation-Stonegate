...
//Recordset: rs_CQWUpdate;
//Searchpage: MasterCQWList.php;
//Form: form1;
...
if ((isset($_POST["WADbSearch1"])) && ($_POST["WADbSearch1"] != "")) {
  ...
  $WADbSearch1->addComparisonFromList("ID","CQW_ID_","AND","=",1);

  //save the query in a session variable
  if (1 == 1) {
    $_SESSION["WADbSearch1_MasterCQWUpdate"]=$WADbSearch1->whereClause;
  }
}
else     {
  $WADbSearch1 = new FilterDef;
  ...
  if (1 == 1)     {
    if (isset($_SESSION["WADbSearch1_MasterCQWUpdate"]) && $_SESSION["WADbSearch1_MasterCQWUpdate"] != "")     {
      $WADbSearch1->whereClause = $_SESSION["WADbSearch1_MasterCQWUpdate"];
    }
    else     {
...
$query_rs_CQWUpdate = "SELECT * FROM admin_media_cqw";
setQueryBuilderSource($query_rs_CQWUpdate,$WADbSearch1,false);
$rs_CQWUpdate = mysql_query($query_rs_CQWUpdate, $GoCreate) or die(mysql_error());
$row_rs_CQWUpdate = mysql_fetch_assoc($rs_CQWUpdate);
$totalRows_rs_CQWUpdate = mysql_num_rows($rs_CQWUpdate);?>
<?php
...
if (isset($_POST["Update"])) // Trigger
{
  ...
  $WA_table = "admin_media_cqw";
  $WA_redirectURL = "MasterCQWList.php";
  if (function_exists("rel2abs")) $WA_redirectURL = $WA_redirectURL?rel2abs($WA_redirectURL,dirname(__FILE__)):"";
  $WA_keepQueryString = true;
  $WA_indexField = "ID";
  $WA_fieldNamesStr = "COST|QTY|LIMIT|WEIGHT";
  $WA_columnTypesStr = "none,none,NULL|none,none,NULL|',none,''|none,none,NULL";
  ...
  while (WA_AB_checkLoopedFieldsNotBlank($WA_loopedIDField, $WA_multipleUpdateCounter)) {
    $WA_fieldValuesStr = "".WA_AB_getLoopedFieldValue("Cost", $WA_multipleUpdateCounter)  ."" . "|" . "".WA_AB_getLoopedFieldValue("QTY", $WA_multipleUpdateCounter)  ."" . "|" . "".WA_AB_getLoopedFieldValue("SetQTY", $WA_multipleUpdateCounter)  ."" . "|" . "".WA_AB_getLoopedFieldValue("Weight", $WA_multipleUpdateCounter)  ."";
        ...
        <div class='content cssLI GoCreateAdmin1_content_design'>
          ...
          <div class='cssLO GoCreateAdmin1_centercolumn_layout'>
            <div class='centercolumn cssLI GoCreateAdmin1_centercolumn_design'>
              ...
              <div id="HeaderTitle">Master C/Q/W Update for <span class="HighLight"><?php echo $_GET['MT'];?></span> size <span class="HighLight" ><?php echo $_GET['MS'];?></span></div>
              <div>
...
<?php
	...
	while($RepeatSelectionCounter_1_IterationsRemaining--){
		if($RepeatSelectionCounterBasedLooping_1 || $row_rs_CQWUpdate){
?>
                   
                    <tr align="right" class="roweffect">
                      <td class="form_txt"><input type="hidden" name="WADA_RepeatID_ID_<?php echo $RepeatSelectionCounter_1; ?>" id="WADA_RepeatID_ID_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWUpdate["ID"]; ?>" />
                      <input name="CQW_ID_<?php echo $RepeatSelectionCounter_1; ?>" type="hidden" id="CQW_ID_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWUpdate['ID']; ?>" /><?php echo $RepeatSelectionCounter_1+1; ?></td>
                      <td class="form_txt">&pound;&nbsp;
                        <input name="Cost_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="Cost_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWUpdate['COST']; ?>" size="12" /></td>
                      <td><input name="QTY_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="QTY_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWUpdate['QTY']; ?>" size="12" /></td>
                      <td><select name="SetQTY_<?php echo $RepeatSelectionCounter_1; ?>" class="form" id="SetQTY_<?php echo $RepeatSelectionCounter_1; ?>">
                        <option selected="selected" value="" <?php if (!(strcmp("", $row_rs_CQWUpdate['LIMIT']))) {echo "selected=\"selected\"";} ?>>Select..</option>
                        <option value="SetQTY" <?php if (!(strcmp("SetQTY", $row_rs_CQWUpdate['LIMIT']))) {echo "selected=\"selected\"";} ?>>Set Quantity</option>
                        <option value="UpToInc" <?php if (!(strcmp("UpToInc", $row_rs_CQWUpdate['LIMIT']))) {echo "selected=\"selected\"";} ?>>Up To &amp; Inc</option>
                        <option value="Single" <?php if (!(strcmp("Single", $row_rs_CQWUpdate['LIMIT']))) {echo "selected=\"selected\"";} ?>>Single</option>
                      </select></td>
                      <td><input name="Weight_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="Weight_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_CQWUpdate['WEIGHT']; ?>" size="12" />
                        <span class="form_txt">(Kg)</span></td>
                    </tr>                    
<?php
...
<?php } // RepeatSelectionCounter_1 End Alternate Content
		if(!$RepeatSelectionCounterBasedLooping_1 && $RepeatSelectionCounter_1_IterationsRemaining != 0){
			if(!$row_rs_CQWUpdate && $RepeatSelectionCounter_1_Iterations == -1){$RepeatSelectionCounter_1_IterationsRemaining = 0;}
			$row_rs_CQWUpdate = mysql_fetch_assoc($rs_CQWUpdate);
		}
...
?>
                  <tr>
                      <td colspan="5" align="right" class="form_txt"><input name="Update" type="submit" class="cust_button" id="Update" value="Update" />
                      &nbsp;
                      <input name="Cancel" type="button" class="cust_button" id="Cancel" onclick="MM_goToURL('parent','MasterCQWList.php?<?php echo $_SERVER['QUERY_STRING'];?>');return document.MM_returnValue" value="Cancel" /></td>
                    </tr>
...
<?php
mysql_free_result($rs_CQWUpdate);
?>
