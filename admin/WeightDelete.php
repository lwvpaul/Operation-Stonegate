...
if ((isset($_POST["WADbSearch1"])) && ($_POST["WADbSearch1"] != "")) {
  ...
  if (1 == 1) {
    $_SESSION["WADbSearch1_WeightDelete"]=$WADbSearch1->whereClause;
  }
}
else     {
  $WADbSearch1 = new FilterDef;
  ...
  if (1 == 1)     {
    if (isset($_SESSION["WADbSearch1_WeightDelete"]) && $_SESSION["WADbSearch1_WeightDelete"] != "")     {
      $WADbSearch1->whereClause = $_SESSION["WADbSearch1_WeightDelete"];
    }
    else     {
...
$query_rs_WeightDelete = "SELECT * FROM shipping";
setQueryBuilderSource($query_rs_WeightDelete,$WADbSearch1,false);
$rs_WeightDelete = mysql_query($query_rs_WeightDelete, $GoCreate) or die(mysql_error());
$row_rs_WeightDelete = mysql_fetch_assoc($rs_WeightDelete);
$totalRows_rs_WeightDelete = mysql_num_rows($rs_WeightDelete);?>
<?php
...
<!-- leftcolumn Content Ends Here -->
          ...
          <div class='cssLO GoCreateAdmin1_centercolumn_layout'>
            <div class='centercolumn cssLI GoCreateAdmin1_centercolumn_design'>
              ...
              <div id="HeaderTitle">Master Weight Delete</div>
	...
	while($RepeatSelectionCounter_1_IterationsRemaining--){
		if($RepeatSelectionCounterBasedLooping_1 || $row_rs_WeightDelete){
?>   
                    <tr align="center" class="roweffect">
                      <td class="form_txt"><input type="hidden" name="WADA_RepeatID_ID_<?php echo $RepeatSelectionCounter_1; ?>" id="WADA_RepeatID_ID_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_WeightDelete["ID"]; ?>" />
                        <input name="WeightID_<?php echo $RepeatSelectionCounter_1; ?>" type="hidden" id="WeightID_<?php echo $RepeatSelectionCounter_1; ?>" value='<?php echo $row_rs_WeightDelete["ID"]; ?>' />
<?php echo $RepeatSelectionCounter_1+1; ?>.</td>
                      <td class="form_txt"><input type="hidden" name="WeightFrom_mihidden_<?php echo $RepeatSelectionCounter_1; ?>" id="WeightFrom_mihidden_<?php echo $RepeatSelectionCounter_1; ?>" value="1" />
                        <input name="WeightFrom_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="WeightFrom_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_WeightDelete['WEIGHT_FROM']; ?>" size="8" />
                        Kg</td>
                      <td class="form_txt"><input name="WeightTo_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="WeightTo_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_WeightDelete['WEIGHT_TO']; ?>" size="8" />
                        Kg</td>
                      <td class="form_txt">&pound;
                        <input name="WeightCost_<?php echo $RepeatSelectionCounter_1; ?>" type="text" class="form" id="WeightCost_<?php echo $RepeatSelectionCounter_1; ?>" value="<?php echo $row_rs_WeightDelete['PRICE']; ?>" size="8" /></td>
                    </tr>
<?php
...
<?php } // RepeatSelectionCounter_1 End Alternate Content
		if(!$RepeatSelectionCounterBasedLooping_1 && $RepeatSelectionCounter_1_IterationsRemaining != 0){
			if(!$row_rs_WeightDelete && $RepeatSelectionCounter_1_Iterations == -1){$RepeatSelectionCounter_1_IterationsRemaining = 0;}
			$row_rs_WeightDelete = mysql_fetch_assoc($rs_WeightDelete);
		}
...
?>                    
  <tr>
    ...
    <td colspan="3" align="left" class="topborder"><input name="ConfirmDelete" type="submit" class="cust_button" id="ConfirmDelete" value="ConfirmDelete" />
      <input name="Cancel" type="button" class="cust_button" id="Cancel" onclick="MM_goToURL('parent','WeightList.php');return document.MM_returnValue" value="Cancel" /></td>
              ...
              </form><!-- centercolumn Content Ends Here -->
              </div>
          </div>
...
<?php
mysql_free_result($rs_WeightDelete);
?>
